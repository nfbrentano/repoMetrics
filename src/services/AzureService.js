...
export class AzureService extends BaseGitService {
  ...
  async fetchAll(rangeInMonths = 6) {
    ...
    const results = {
      ...
      prsOpen: prs.value ? prs.value.filter(p => p.status === 'active').length : 0,
      issuesOpen: 0, // Azure Devops Issues (Work Items) require a different API
      issuesClosed: 0,
      issuesTotal: 0,
      lastUpdate: commits.value && commits.value.length > 0 && commits.value[0].author?.date ? commits.value[0].author.date : null,
      authors: {},
      reviews: 0,
      totalReviewTime: 0,
      pullsWithReview: 0,
